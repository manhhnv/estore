...
import { changeDefaultAddress, resetAddress } from 'estore/redux/slice/addressSlice';
...
const ListAddress = ({ address, changeDefaultAddress, resetAddress }: ListAddressProps) => {
    const [
        executeGetAddresses,
    ...
    useEffect(() => {
        if (
            (data && data.getUserAddresses && data.getUserAddresses.totalItems == 0)
        ) {
            resetAddress ? resetAddress(null) : null
        }
        else if (data && data.getUserAddresses && data.getUserAddresses.totalItems && data.getUserAddresses.totalItems > 0) {
...
export default connect(mapStateToProps, mapDispatchToProps)(React.memo(ListAddress));
