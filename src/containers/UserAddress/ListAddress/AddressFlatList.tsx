...
import { Address, useRemoveUserAddressMutation } from 'estore/graphql/generated';
import { View, FlatList, ActivityIndicator, Dimensions, TouchableOpacity, Text, ToastAndroid } from 'react-native';
...
export const AddressFlatList = ({ addresses, getUserAddresses, navigation }: AddressFlatListProps) => {
    const [
        executeGQL,
        {
            called,
    ...
    const removeAddressCallback = useCallback((id: string) => {
    ...
    const renderItem = ({ item }: { item: Partial<Address | null> }) => {
        if (item) {
            return <AddressItem item={item} removeAddressCallback={removeAddressCallback} />;
        }
        return <View></View>;
    };
    useEffect(() => {
